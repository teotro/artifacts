...
def enumerate_all_connections_backtrack_dfs(trace_matrix):
    ...
    def constrained_paths(u, v, used):
        ...
        def dfs(path, seen_wires):
            ...
            if node == v:
                print(f"Path found: {path} (seen_wires={seen_wires})")
                # Accept path if it includes at least one wire-tile OR is a direct neighbor (length == 2)
    ...
    def backtrack(pairs_left, used, current_solution):
        ...
        for v in pairs_left[1:]:
            ...
            print(f"Trying pair: {u} <-> {v}")
            for path in constrained_paths(u, v, used):
                print(f"  Path for {u} <-> {v}: {path}")
                path_set = set(path)
...
def filter_same_length_duplicates(solutions):
    """
    From a list of [((q1,q2), path1), …] solutions, keep
    exactly one per unique set of endpoint‐pairs, ignoring path.
    """
    def canonical_pair(u, v):
        # convert to ints; return (u,v) sorted so (a,b)==(b,a)
        u = (int(u[0]), int(u[1]))
        v = (int(v[0]), int(v[1]))
    ...
    for sol in solutions:
        # signature = frozenset of ordered pairs
        sig = frozenset(canonical_pair(*pair) for pair, _ in sol)
        if sig in seen:
            continue
        seen.add(sig)

        # rebuild coords as plain ints
        clean_sol = [
            ((int(u[0]), int(u[1])), (int(v[0]), int(v[1])), path)
            for (u, v), path in sol
        ]
        # repackage to [ ((q1,q2), path), … ]
        deduped.append([((u, v), p) for u, v, p in clean_sol])
...
def _worker_pair_and_backtrack(args):
    ...
    qubits = [tuple(rc) for rc in np.argwhere(trace_matrix == 0b1110000)]
    print(f"[Worker] Initial endpoints list: {qubits}")
    print(f"[Worker] Initial fixed pair: {u} <-> {v}")
    ...
    def constrained_paths(u0, v0, used0):
        ...
        def dfs(path, seen_wire):
            ...
            #     return
            if node == v0:
                print(f"Path found: {path} (seen_wire={seen_wire})")
    ...
    def backtrack(pairs_left, used, curr_solution):
        ...
        if not pairs_left:
            if used == target_cover:
                print("\n=== FULL SOLUTION FOUND ===")
                for (pair, path) in curr_solution:
                    print(f"  Pair: {pair}, Path: {path}")
                print("  Used tiles:", sorted(used))
                print("  Target wire-tiles:", sorted(target_cover))
                print("  Are all wire-tiles covered?", used == target_cover)
                solutions.append(curr_solution.copy())
            else:
                print("\n--- Partial solution (NOT covering all wire-tiles) ---")
                for (pair, path) in curr_solution:
                    print(f"  Pair: {pair}, Path: {path}")
                print("  Used tiles:", sorted(used))
                print("  Target wire-tiles:", sorted(target_cover))
                print("  Missing:", sorted(target_cover - used))
                visited_fails.add(key)
            return
        ...
        for v1 in pairs_left[1:]:
            ...
            print("Trying to pair:", u1, v1)    # <--- *** ADD HERE ***
...
# def reconstruct_dag_from_l3_traces(recovered_l3_traces, layout_type, benchmark_name):
    import numpy as np
    import networkx as nx
    ...
    # for step_str, entry in sorted(refined_l3_traces.items(), key=lambda x: int(x[0])):
        ...
        if ambiguous:
            ...
            if qubits and wires:
                ...
                valid_routes = filter_non_overlapping_paths(raw)
                non_overlapping   = filter_same_length_duplicates(valid_routes)
                # non_overlapping   = valid_routes
...
# Uncomment these if you want timing data
    import json
    import os
    ...
    base_dir = f"/home/george/artifacts/traceq/targets/" # Replace this with your own path!
    # base_dir = f"/home/george/FTQC_Security/queries_3/"
